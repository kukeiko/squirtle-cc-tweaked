...
function MessagePump.run(...)
    ...
    if (MessagePump._isRunning) then
        return true
    end
    ...
    while (MessagePump._numCoroutines > 0 and not MessagePump._doQuit) do
        ...
        --        Log.debug("------------------------------------------------")
        --        Log.debug("[MessagePump] pulled " .. string.gsub(textutils.serialize(eventData), "[\n ]", ""))
        for coroId, coro in pairs(coroutines) do
            ...
            if (filter == nil or eventName == filter) then
                ...
                if (not success) then
                    Log.error("[MessagePump] a coroutine died: " .. param)
                    MessagePump.reset()
                    error(param, 0)
                end
